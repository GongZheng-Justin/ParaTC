module m_LogInfo
  use MPI
  use m_TypeDef,only:GetFileUnit,nrank 
  implicit none
  private
...
contains
  ...
  subroutine LI_OpenFile(this,chFile,RunName)
    ...
    if(nrank==0) then
      open(file = chFile, unit =this%nUnit, status='replace', IOSTAT=myistat)
      write(this%nUnit , *, IOSTAT=myistat ) "**************************************************************************"
      write(this%nUnit , *, IOSTAT=myistat ) "Log file for run: "// trim(RunName)
      write(this%nUnit , *, IOSTAT=myistat ) "**************************************************************************"
      write(this%nUnit , *, IOSTAT=myistat )
    endif
    call MPI_BARRIER(MPI_COMM_WORLD,myistat)
    if(nrank/=0) open(file = chFile, unit =this%nUnit, status='old', IOSTAT=myistat)
  end subroutine 
