...
dir_statOut='../../StatOut/';
yMesh_str=[dir_statIn,'yMeshForCha550_04.txt'];
...
for k=1:nxhp
  ...
  for jc=1:nyc
    ...
    rTemp=atan(SpectraX(k,jc,8)/SpectraX(k,jc,7));
    rTemp=waveNumber*YMinusYr/rTemp;
    rTemp=atan(rTemp)*180/pi;
    SpectraX(k,jc,8)=rTemp;  
  end
  fprintf(fid,myformat,[waveNumber,SpectraX(k,:,8)]);
end
fclose(fid);
...
for k=1:nxhp
  for jc=1:nyc
    SpectraX(k,jc,7)=4.0*SpectraX(k,jc,7)^2/(SpectraX(k,jForLCS,1)*SpectraX(k,jc,1));  
  end
  fprintf(fid,myformat,[2*pi/xlx*(k-1),SpectraX(k,:,7)]);
end
fclose(fid);
end
...
for k=1:nzhp
  ...
  for jc=1:nyc
    YMinusYr=abs(yc(jc)-yc(jForLCS));
    rTemp=atan(SpectraZ(k,jc,8)/SpectraZ(k,jc,7));
    rTemp=waveNumber*YMinusYr/rTemp;
    rTemp=atan(rTemp)*180/pi;
    SpectraZ(k,jc,8)=rTemp;  
  end
  fprintf(fid,myformat,[waveNumber,SpectraZ(k,:,8)]);
end
fclose(fid);
...
for k=1:nzhp
  for jc=1:nyc
    SpectraZ(k,jc,7)=4.0*SpectraZ(k,jc,7)^2/(SpectraZ(k,jForLCS,1)*SpectraZ(k,jc,1));  
  end
  fprintf(fid,myformat,[2*pi/zlz*(k-1),SpectraZ(k,:,7)]);
end
fclose(fid);
end