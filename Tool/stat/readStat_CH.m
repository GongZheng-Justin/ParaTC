...
%   * specx_uu.txt: 1D uu spectra in x-dir                                %
%   * specx_vv.txt: 1D vv spectra in x-dir                                %
%   * specx_ww.txt: 1D ww spectra in x-dir                                %
%   * specx_pp.txt: 1D pp spectra in x-dir                                %
%   * specz_uu.txt: 1D uu spectra in z-dir                                %
%   * specz_vv.txt: 1D vv spectra in z-dir                                %
%   * specz_ww.txt: 1D ww spectra in z-dir                                %
%   * specz_pp.txt: 1D p spectra in z-dir                                 %
%   * specx_uv.txt: 1D uv cospectra in x-dir                              %
%   * specz_uv.txt: 1D uv cospectra in z-dir                              %
%   * LCS_uu_x.txt: LCS for uu in x-dir                                   %         
%   * LCS_uu_z.txt: LCS for uu in z-dir                                   %
%   * LCS_uu_theta_x.txt: Attached angle for uu in x-dir from LCS         %
%   * LCS_uu_theta_z.txt: Attached angle for uu in z-dir from LCS         %
%                                                                         %
% Author:                                                                 %
...
jForLCS =18;    % Reference j-index for Linear coherent structure

dir_statIn = '../../CFD/Results/';
dir_statOut='../../StatOut/';
yMesh_str=[dir_statIn,'yMeshForCha550L_01.txt'];
...
for k=1:nxhp
  ...
  for m=1:nych
    msc=nyc+1-m;
    SpecData(m)=0.5*(SpectraX(k,m  ,1)+SpectraX(k,msc,1));
  end
...
for k=1:nxhp
  ...
  for m=1:nych
    mt=m+1;
    msc=nyp+1-mt;
    SpecData(m)=0.5*(SpectraX(k,mt ,2)+SpectraX(k,msc,2));
  end
  fprintf(fid,myformat,[2*pi/xlx*(k-1),SpecData]);
end
fclose(fid);
...
for k=1:nxhp
  ...
  for m=1:nych
    msc=nyc+1-m;
    SpecData(m)=0.5*(SpectraX(k,m  ,3)+SpectraX(k,msc,3));
  end
  fprintf(fid,myformat,[2*pi/xlx*(k-1),SpecData]);
end
fclose(fid);
...
for k=1:nxhp
  ...
  for m=1:nych
    msc=nyc+1-m;
    SpecData(m)=0.5*(SpectraX(k,m  ,4)+SpectraX(k,msc,4));
  end
  fprintf(fid,myformat,[2*pi/xlx*(k-1),SpecData]);
end
fclose(fid);
...
for k=1:nxhp
  ...
  for m=1:nych
    msc=nyc+1-m;
    SpecData(m)=0.5*(SpectraX(k,m  ,6)-SpectraX(k,msc,6));
  end
  fprintf(fid,myformat,[2*pi/xlx*(k-1),SpecData]);
end
fclose(fid);
...
for k=1:nxhp
  ...
  for m=1:nych
    msc=nyc+1-m;
    SpecData(m)=0.5*(SpectraX(k,m  ,5)-SpectraX(k,msc,5));
  end
  fprintf(fid,myformat,[2*pi/xlx*(k-1),SpecData]);
end
fclose(fid);
...
% LCS for uu in x-dir
fid=fopen([dir_statOut,'LCS_uu_theta_x.txt'],'wt');
fprintf(fid,myformat,[Retau,yplus_spec_center]);
for k=1:nxhp
  jt=nyc+1-jForLCS;
  SpecData=zeros(1,nych);
  ...
  for jc=1:nych
    jsc=nyc+1-jc;
    YMinusYr=abs(yc(jc)-yc(jForLCS));
    rTemp1=-atan(SpectraX(k,jc,8)/SpectraX(k,jc,7));
    rTemp1=waveNumber*YMinusYr/rTemp1;
    rTemp1=atan(rTemp1)*180/pi;    
    rTemp2=-atan(SpectraX(k,jsc,8)/SpectraX(k,jsc,7));
...
fid=fopen([dir_statOut,'LCS_uu_x.txt'],'wt');
fprintf(fid,myformat,[Retau,yplus_spec_center]);
for k=1:nxhp
  jt=nyc+1-jForLCS;
  SpecData=zeros(1,nych);
  for jc=1:nych
    jsc=nyc+1-jc;
    rTemp1=4.0*SpectraX(k,jc, 7)^2/(SpectraX(k,jForLCS,1)*SpectraX(k,jc,1));  
    rTemp2=4.0*SpectraX(k,jsc,7)^2/(SpectraX(k,jt,1)    *SpectraX(k,jsc,1));
    SpecData(jc)=0.5*(rTemp1+rTemp2);
  end
  fprintf(fid,myformat,[2*pi/xlx*(k-1),SpecData]);
end
...
for k=1:nzhp
  ...
  for m=1:nych
    msc=nyc+1-m;
    SpecData(m)=0.5*(SpectraZ(k,m  ,1)+SpectraZ(k,msc,1));
  end
...
for k=1:nzhp
  ...
  for m=1:nych
    mt=m+1;
    msc=nyp+1-mt;
    SpecData(m)=0.5*(SpectraZ(k,mt ,2)+SpectraZ(k,msc,2));
  end
  fprintf(fid,myformat,[2*pi/zlz*(k-1),SpecData]);
end
fclose(fid);
...
for k=1:nzhp
  ...
  for m=1:nych
    msc=nyc+1-m;
    SpecData(m)=0.5*(SpectraZ(k,m  ,3)+SpectraZ(k,msc,3));
  end
  fprintf(fid,myformat,[2*pi/zlz*(k-1),SpecData]);
end
fclose(fid);
...
for k=1:nzhp
  ...
  for m=1:nych
    msc=nyc+1-m;
    SpecData(m)=0.5*(SpectraZ(k,m  ,4)+SpectraZ(k,msc,4));
  end
  fprintf(fid,myformat,[2*pi/zlz*(k-1),SpecData]);
end
fclose(fid);
...
for k=1:nzhp
  ...
  for m=1:nych
    msc=nyc+1-m;
    SpecData(m)=0.5*(SpectraZ(k,m  ,6)-SpectraZ(k,msc,6));
  end
  fprintf(fid,myformat,[2*pi/zlz*(k-1),SpecData]);
end
fclose(fid);
...
for k=1:nzhp
  ...
  for m=1:nych
    msc=nyc+1-m;
    SpecData(m)=0.5*(SpectraZ(k,m  ,5)-SpectraZ(k,msc,5));
  end
  fprintf(fid,myformat,[2*pi/zlz*(k-1),SpecData]);
end
fclose(fid);
...
% LCS for uu in z-dir
fid=fopen([dir_statOut,'LCS_uu_theta_z.txt'],'wt');
fprintf(fid,myformat,[Retau,yplus_spec_center]);
for k=1:nzhp
  jt=nyc+1-jForLCS;
  SpecData=zeros(1,nych);
  ...
  for jc=1:nych
    jsc=nyc+1-jc;
    YMinusYr=abs(yc(jc)-yc(jForLCS));
    rTemp1=-atan(SpectraZ(k,jc,8)/SpectraZ(k,jc,7));
    rTemp1=waveNumber*YMinusYr/rTemp1;
    rTemp1=atan(rTemp1)*180/pi;    
    rTemp2=-atan(SpectraZ(k,jsc,8)/SpectraZ(k,jsc,7));
    rTemp2=waveNumber*YMinusYr/rTemp2;
    rTemp2=atan(rTemp2)*180/pi;
    SpecData(jc)=0.5*(rTemp1+rTemp2);
  end
...
fid=fopen([dir_statOut,'LCS_uu_z.txt'],'wt');
fprintf(fid,myformat,[Retau,yplus_spec_center]);
for k=1:nzhp
  jt=nyc+1-jForLCS;
  SpecData=zeros(1,nych);
  for jc=1:nych
    jsc=nyc+1-jc;
    rTemp1=4.0*SpectraZ(k,jc, 7)^2/(SpectraZ(k,jForLCS,1)*SpectraZ(k,jc,1));  
    rTemp2=4.0*SpectraZ(k,jsc,7)^2/(SpectraZ(k,jt,1)    *SpectraZ(k,jsc,1));
    SpecData(jc)=0.5*(rTemp1+rTemp2);
  end
  fprintf(fid,myformat,[2*pi/zlz*(k-1),SpecData]);
end
