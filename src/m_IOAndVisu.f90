...
contains
  ...
  subroutine InitVisu(ChannelPrm)
    ...
    if(XDMF_SET_TYPE==0) then ! 0: Cell
      write(XDMF_SET_TYPE_STR,"(A)")'" Center="Cell">'
      nxOut=nxp; nyOut=nyp; nzOut=nzp
    else                      ! 1: Node
      write(XDMF_SET_TYPE_STR,"(A)")'" Center="Node">'
      nxOut=nxc; nyOut=nyc; nzOut=nzc
    endif

    nUnitFile = GetFileUnit()
    write(xdmfFile,"(A)") trim(Res_Dir)//"VisuFor"//trim(RunName)//".xmf"
  ...
  subroutine dump_visu(ntime,ux,uy,uz,pressure,ArrTemp)
    ...
    if(save_ux) then
      do kc=y1start(3),y1end(3)
        ...
        enddo
      enddo
      write(chFile,"(A,I10.10)") trim(Res_Dir)//"VisuFor"//trim(RunName)//"_ux_",ntime
    ...
    if(save_uy) then
      do kc=y1start(3),y1end(3)
        ...
        enddo
      enddo
      write(chFile,"(A,I10.10)") trim(Res_Dir)//"VisuFor"//trim(RunName)//"_uy_",ntime
      call decomp_2d_write_every(y_pencil,ArrTemp,iskip,jskip,kskip,chFile,from1=.true.)
    endif
    ...
    if(save_uz) then
      do kc=y1start(3),y1end(3)
        ...
        enddo
      enddo
      write(chFile,"(A,I10.10)") trim(Res_Dir)//"VisuFor"//trim(RunName)//"_uz_",ntime
      call decomp_2d_write_every(y_pencil,ArrTemp,iskip,jskip,kskip,chFile,from1=.true.)
    endif

    ! pressure
    if(save_pr) then
      write(chFile,"(A,I10.10)") trim(Res_Dir)//"VisuFor"//trim(RunName)//"_pr_",ntime
...
#ifdef ScalarFlow
    if(save_scalar) then
      write(chFile,"(A,I10.10)") trim(Res_Dir)//"VisuFor"//trim(RunName)//"_scalar_",ntime
    ...
    if(save_wx) then
      do kc=y1start(3),y1end(3)
        ...
        enddo
      enddo
      write(chFile,"(A,I10.10)") trim(Res_Dir)//"VisuFor"//trim(RunName)//"_wx_",ntime
      call decomp_2d_write_every(y_pencil,ArrTemp,iskip,jskip,kskip,chFile,from1=.true.)
    endif
    ...
    if(save_wy) then
      do kc=y1start(3),y1end(3)
        ...
        enddo
      enddo
      write(chFile,"(A,I10.10)") trim(Res_Dir)//"VisuFor"//trim(RunName)//"_wy_",ntime
      call decomp_2d_write_every(y_pencil,ArrTemp,iskip,jskip,kskip,chFile,from1=.true.)
    endif
    ...
    if(save_wz) then
      do kc=y1start(3),y1end(3)
        ...
        enddo
      enddo
      write(chFile,"(A,I10.10)") trim(Res_Dir)//"VisuFor"//trim(RunName)//"_wz_",ntime
      call decomp_2d_write_every(y_pencil,ArrTemp,iskip,jskip,kskip,chFile,from1=.true.)
    endif
    ...
    if(save_wMag) then
      do kc=y1start(3),y1end(3)
        ...
        enddo
      enddo
      write(chFile,"(A,I10.10)") trim(Res_Dir)//"VisuFor"//trim(RunName)//"_wMag_",ntime 
      call decomp_2d_write_every(y_pencil,ArrTemp,iskip,jskip,kskip,chFile,from1=.true.)
    endif
    ...
    if(save_Q_vor) then
      ...
      write(chFile,"(A,I10.10)") trim(Res_Dir)//"VisuFor"//trim(RunName)//"_Q_",ntime 
      call decomp_2d_write_every(y_pencil,ArrTemp,iskip,jskip,kskip,chFile,from1=.true.)
    endif  
   ...
   if(save_lamda2) then
     ...
     write(chFile,"(A,I10.10)") trim(Res_Dir)//"VisuFor"//trim(RunName)//"_lamda2_",ntime 
     call decomp_2d_write_every(y_pencil,ArrTemp,iskip,jskip,kskip,chFile,from1=.true.)
   endif
