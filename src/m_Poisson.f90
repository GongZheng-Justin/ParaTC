module m_Poisson
  ...
  procedure(),pointer::clcPPE
  type(decomp_info),allocatable::decomp_PPE
  real(RK),save,allocatable,dimension(:)::WaveNumX,WaveNumZ
  type(C_PTR),save::fwd_plan_x,bwd_plan_x,fwd_plan_z,bwd_plan_z
  real(RK),save,allocatable,dimension(:,:,:)::a_reduce,c_reduce 
...
contains
  ...
  subroutine InitPoissonSolver(prsrc,prphiHalo)
    ...
    if(nrank==0) write(*,*) ' ' 
    if(nrank==0) write(*,*) 'Auto-tuning mode for Poisson Solver......'
    best_time=huge(t1)
    ...
    if(nrank==0)print*,'  Choice-1, time=',t1
    ...
    t2=MPI_WTIME()-t1
    call MPI_ALLREDUCE(t2,t1,1,real_type,MPI_SUM,MPI_COMM_WORLD,iErr01)
    if(nrank==0)print*,'  Choice-2, time=',t1
    if(best_time>t1) then
      best_time=t1
      iChoice=2
    endif
    ...
    if(IsReduceR==1) then
      ...
      t2=MPI_WTIME()-t1
      call MPI_ALLREDUCE(t2,t1,1,real_type,MPI_SUM,MPI_COMM_WORLD,iErr01)
      if(nrank==0)print*,'  Choice-3, time=',t1
      if(best_time>t1) then
        best_time=t1
        iChoice=3
      endif
      ...
      if(nrank==0)print*,'  Choice-3, z2size<4, ignore'
    endif
    call Destory_Poisson_FFT_Plan()
    ...
    if(IsReduceR==1) then   
      ...
      t2=MPI_WTIME()-t1
      call MPI_ALLREDUCE(t2,t1,1,real_type,MPI_SUM,MPI_COMM_WORLD,iErr01)
      if(nrank==0)print*,'  Choice-4, time=',t1
      if(best_time>t1) then
        best_time=t1
        iChoice=4
      endif
      ...
      if(nrank==0)print*,'  Choice-4, x2size<4, ignore'
    endif
    call Destory_Poisson_FFT_Plan()
    
    if(nrank==0) then
      write(*,'(A,I1)')' The best Poisson Solver choice is probably Choice-',iChoice 
      print*,'Corresponding Global Data Transpose is: '
    endif
    if(iChoice==1) then
      clcPPE => clcPPE_x1
      call Create_Poisson_FFT_Plan(x1size,z2size)
      if(nrank==0) print*,'  y1 -> x1 -> z2 -> y2 -> z2 -> x1 -> y1'
    elseif(iChoice==2) then
      clcPPE => clcPPE_z1 
      call Create_Poisson_FFT_Plan(x2size,z1size)
      if(nrank==0) print*,'  y1 -> z1 -> x2 -> y2 -> x2 -> z1 -> y1'
    elseif(iChoice==3) then
      clcPPE => clcPPE_x1_reduce 
      call Create_Poisson_FFT_Plan(x1size,z2size)
      if(nrank==0) print*,'  y1 -> x1 -> z2 -> x1 -> y1'
      allocate(decomp_PPE)
      allocate(a_reduce(y2start(1):y2end(1),2*p_row,y2start(3):y2end(3)))
      allocate(c_reduce(y2start(1):y2end(1),2*p_row,y2start(3):y2end(3)))
      call Initialize_ReduceMatrix(z2start,z2end,'z')
    elseif(iChoice==4) then
      ...
      call Create_Poisson_FFT_Plan(x2size,z1size)
      if(nrank==0) print*,'  y1 -> z1 -> x2 -> z1 -> y1'
      allocate(decomp_PPE)
      allocate(a_reduce(y2start(1):y2end(1),2*p_col,y2start(3):y2end(3)))
      allocate(c_reduce(y2start(1):y2end(1),2*p_col,y2start(3):y2end(3)))
      call Initialize_ReduceMatrix(x2start,x2end,'x')
    ...
    do kc=startIn(3),endIn(3)
      do jc=startIn(2),endIn(2)
        do ic=startIn(1),endIn(1)
          ArrM(ic,jc,kc)=am2ph(jc)
          ArrC(ic,jc,kc)=ac2ph(jc)+WaveNumX(ic)+WaveNumZ(kc)
          ArrP(ic,jc,kc)=ap2ph(jc)
        enddo
      enddo
    enddo
    IF(nrank==0) THEN
  ...
  subroutine clcPPE_x1(prsrc, prphiHalo)
    ...
    IF(nrank==0) THEN
      do kc=2,y2end(3)
        do jc=1,nyc
          do ic=1,y2end(1)
            tridcj(ic,jc)=ac2ph(jc)+WaveNumX(ic)+WaveNumZ(kc)
          enddo
        enddo
