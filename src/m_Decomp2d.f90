...
#define DECOMP_TRANSPOSE_Z2X1 6
  ...
  logical,save::IsInitTopoEnd=.false.
  integer,save::np_row,np_col,pcoord(2)
  integer,allocatable,dimension(:)::tran_count,tran_disp
  integer,save,public::DECOMP_2D_COMM_ROW,DECOMP_2D_COMM_COL
  ...
  integer,save,dimension(3),public::x1start,x1end,x1size  ! x-pencil
  integer,save,dimension(3),public::y1start,y1end,y1size  ! y-pencil
  integer,save,dimension(3),public::z1start,z1end,z1size  ! z-pencil
  integer,save,dimension(3),public::x2start,x2end,x2size  ! x-pencil
  integer,save,dimension(3),public::y2start,y2end,y2size  ! y-pencil
  integer,save,dimension(3),public::z2start,z2end,z2size  ! z-pencil
  ...
  type(decomp_info),target,save::decomp_main ! main (default) decomposition information
...
contains
  ...
  subroutine decomp_info_init(nx,ny,nz,decomp,np_row_col,initialize)
    ...
    ! x1 <-> y1
    if(decomp%initialize(DECOMP_TRANSPOSE_X1Y1)) then
      allocate(decomp%xtype_x1y1(p_row))
      sizes=decomp%x1sz
      ...
      enddo
      allocate(decomp%ytype_x1y1(p_row))
      sizes=decomp%y1sz
    ...
    ! y1 <-> z1
    if(decomp%initialize(DECOMP_TRANSPOSE_Y1Z1)) then
      allocate(decomp%ytype_y1z1(p_col))
      sizes=decomp%y1sz
      subsizes=decomp%y1sz
      starts=0; istart=0
      do k=1,p_col
      ...
      enddo
      allocate(decomp%ztype_y1z1(p_col))
      sizes=decomp%z1sz
    ...
    ! z1 <-> x2
    if(decomp%initialize(DECOMP_TRANSPOSE_Z1X2)) then
      allocate(decomp%ztype_z1x2(p_row))
      sizes=decomp%z1sz
      subsizes=decomp%z1sz
      starts=0; istart=0
      do k=1,p_row
      ...
      enddo
      allocate(decomp%xtype_z1x2(p_row))
      sizes=decomp%x2sz
    ...
    ! x2 <-> y2
    if(decomp%initialize(DECOMP_TRANSPOSE_X2Y2)) then
      allocate(decomp%xtype_x2y2(p_col))
      sizes=decomp%x2sz
      subsizes=decomp%x2sz
      starts=0; istart=0
      do k=1,p_col
      ...
      enddo
      allocate(decomp%ytype_x2y2(p_col))
      sizes=decomp%y2sz
    ...
    ! y2 <-> z2
    if(decomp%initialize(DECOMP_TRANSPOSE_Y2Z2)) then
      allocate(decomp%ytype_y2z2(p_row))
      sizes=decomp%y2sz
      subsizes=decomp%y2sz
      starts=0; istart=0  
      do k=1,p_row
      ...
      enddo
      allocate(decomp%ztype_y2z2(p_row))
      sizes=decomp%z2sz
    ...
    ! z2 <-> x1
    if(decomp%initialize(DECOMP_TRANSPOSE_Z2X1)) then
      allocate(decomp%ztype_z2x1(p_col))
      sizes=decomp%z2sz
      subsizes=decomp%z2sz
      starts=0; istart=0  
      do k=1,p_col
      ...
      enddo    
      allocate(decomp%xtype_z2x1(p_col))
      sizes=decomp%x1sz
      subsizes=decomp%x1sz
      starts=0; istart=0   
      do k=1,p_col
  ...
  subroutine transpose_x1_to_y1(src, dst, opt_decomp)
    ...
    np=decomp%nrow
    allocate(tran_count(np),tran_disp(np))
  ...
  subroutine decomp_2d_write_plane(ipencil,var,iplane,n,filename,opt_decomp)
    ...
    if(iplane==1)then
      allocate(wk2d(1,decomp%x1sz(2),decomp%x1sz(3)))
      IF(ipencil==1)THEN
      ...
      ELSEIF(ipencil==2)then
        allocate(wk3d(decomp%x1sz(1),decomp%x1sz(2),decomp%x1sz(3)))
        ...
        starts  =[1,decomp%x1st(2),decomp%x1st(3)]-1
      ELSE
        allocate(wk3d(decomp%x2sz(1),decomp%x2sz(2),decomp%x2sz(3)))
    ...
    elseif(iplane==2) then
      allocate(wk2d(decomp%y1sz(1),1,decomp%y1sz(3)))
      IF(ipencil==2)THEN
        ...
        enddo
      ELSE
        allocate(wk3d(decomp%y1sz(1),decomp%y1sz(2),decomp%y1sz(3)))
        if(ipencil==1)then
    ...
    elseif(iplane==3)then
      IF(ipencil==1) THEN
        allocate(wk2d(decomp%z2sz(1),decomp%z2sz(2),1))
        allocate(wk3d(decomp%z2sz(1),decomp%z2sz(2),decomp%z2sz(3)))
      ...
      ELSEIF(ipencil==2) THEN
        allocate(wk2d(decomp%z1sz(1),decomp%z1sz(2),1))
        allocate(wk3d(decomp%z1sz(1),decomp%z1sz(2),decomp%z1sz(3)))
  ...
  subroutine best_2d_grid(nx,ny,nz,pencil,iproc,best_p_row,best_p_col,BcOptions)
    ...
    do i=1,nfact
      ...
      if(min(nx,ny,nz)>=row .and. min(ny,nz,nz)>=col) then
        ...
        allocate(Arr1(decomp%x1sz(1),decomp%x1sz(2),decomp%x1sz(3)))
        allocate(Arr2(decomp%y1sz(1),decomp%y1sz(2),decomp%y1sz(3)))
        ...
        deallocate(Arr1)
        allocate(Arr1(decomp%z1sz(1),decomp%z1sz(2),decomp%z1sz(3)))        
        call MPI_BARRIER(MPI_COMM_WORLD,ierror)
        t1 = MPI_WTIME()
        do k=1,5
        ...
        deallocate(Arr2)     
        allocate(Arr2(decomp%x2sz(1),decomp%x2sz(2),decomp%x2sz(3)))
        call MPI_BARRIER(MPI_COMM_WORLD,ierror)
        t1 = MPI_WTIME()
        do k=1,5
        ...
        deallocate(Arr1)     
        allocate(Arr1(decomp%y2sz(1),decomp%y2sz(2),decomp%y2sz(3)))
        call MPI_BARRIER(MPI_COMM_WORLD,ierror)
        t1 = MPI_WTIME()
        do k=1,5
        ...
        deallocate(Arr2)     
        allocate(Arr2(decomp%z2sz(1),decomp%z2sz(2),decomp%z2sz(3)))
        call MPI_BARRIER(MPI_COMM_WORLD,ierror)
        t1 = MPI_WTIME()
        do k=1,5
